...
Tcentralbmus = Tcentralb*mu_i*mu**(-2)*mu_e**(5/3)
Tcentralb_solarUnits = Tcentralb*cons.mSun**(4/3)
Tcentralbmus_solarUnits = Tcentralbmus*cons.mSun**(4/3)
print('  When Pgas=Pe, Tcentral = {:.3e} mu_i mu^-2 mu_e^5/3 M^4/3'.format(Tcentralb))
print('       or {:.2e} M^4/3'.format(Tcentralbmus))
print('       or {:.3e} mu_i mu^-2 mu_e^5/3 (M/Msun)^4/3'.format(Tcentralb_solarUnits))
print('       or {:.2e} (M/Msun)^4/3'.format(Tcentralbmus_solarUnits))

Tcentralc = 4e6 #K
print('  mu_i = {:.3e}'.format(mu_i))
Mpart1 = (1.004e13*cons.NA*cons.kB*25*cons.G**(-2))**(3/4)*4**(-5/4)
Mpart2 = np.pi**(-1/2)*x1True[i4]**(2)*z1True[i4]*Tcentralc**(3/4)
M = Mpart1*Mpart2*mu_i**(3/4)*mu_e**(-5/4)*mu**(-3/2)
...
TeffCons = 2**(4/51)*PpCons**(2/17)*KprimeCons**(-2/17)*(1.11)**(-5/17)
TeffCons_solarUnits = TeffCons*cons.mSun**(7/51)*cons.lSun**(1/102)
print('  TeffCons = {:.3e} (Z/0.02)^-4/51 mu^13/51 M^7/51 L^1/102'.format(TeffCons))
print('  TeffCons = {:.3e} (Z/0.02)^-4/51 mu^13/51 (M/Msun)^7/51 (L/Lsun)^1/102'.format(TeffCons_solarUnits))
